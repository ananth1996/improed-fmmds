...
def get_parser():
    ...
    parser.add_argument(
        "-k", type=int, help="The number of samples required", default=10
    )
    parser.add_argument(
        "-C",
        type=int,
        choices=[2, 7, 14],
        help="Number of groups in Census Dataset",
        default=2,
    )
    parser.add_argument(
        "--data_dir", type=Path, default=DATA_DIR, help="Location of data"
    )
    parser.add_argument(
        "--old", action="store_true", help="To use old version of FMMD-S algorithm"
    )
    parser.add_argument(
        "--eps", type=float, default=0.05, help="The factor to relax threshold by"
    )
    parser.add_argument(
        "-d",
        "--debug",
...
if __name__ == "__main__":
    ...
    ids, features, groups = load_census(
        grouping=group_types[args.C], data_dir=args.data_dir
    )
    elements = []
    ...
    old = True
    if args.old:
        solution, diversity, _ = scalable_fmmd_ILP(
            elements, args.eps, args.k, args.C, constraints, utils.euclidean_dist
        )
    else:

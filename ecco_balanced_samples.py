...
def get_parser():
    ...
    parser.add_argument(
        "-k", type=int, help="Minimum number of samples needed")
    parser.add_argument("--eps", type=float,
                        help="The factor to relax threshold by", default=0.05)
    parser.add_argument("--data-dir", type=Path, help="Location of ecco data",
                        default=DATA_DIR)
    parser.add_argument("--output-dir", type=Path, help="Location to store solution",
                        default=DATA_DIR)
...
if __name__ == "__main__":
    ...
    if args.algorithm == "greedy":
        ...
        greedy_solution, _, _ = group_gonzales_algorithm(
            set(), features, ids, groups, num_samples_per_group, args.eps, constraints)
        ...
        if args.pca:
            output_file = args.output_dir/f"greedy_diverse_balanced_samples_pca_{k}.txt"
        else:
